...
namespace CMU462 {
	...
	void SoftwareRendererImp::rasterize_line(float x0, float y0, float x1,
		...
		// Bresenham Algorithm
		int sx0 = (int)floor(x0), sy0 = (int)floor(y0);
		int sx1 = (int)floor(x1), sy1 = (int)floor(y1);
		...
		if (sx0 == sx1) {
			...
			bresenham_line(sx0, sy0, sx1, sy1, color);
		} else {
			bresenham_line(sx1, sy1, sx0, sy0, color);
		}
	}

	void SoftwareRendererImp::bresenham_line(int x0, int y0, int x1, int y1,
											 Color color) {
		int dx = x1 - x0, dy = y1 - y0, x = x0, y = y0, eps = 0;
