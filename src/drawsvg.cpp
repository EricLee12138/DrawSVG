...
namespace CMU462 {
	...
	string DrawSVG::name() { return "DrawSVG"; }
	...
	void DrawSVG::resize(size_t width, size_t height) {
		...
		software_renderer_imp->set_render_target(&framebuffer[0], width,
												 height);
		software_renderer_ref->set_render_target(&framebuffer[0], width,
												 height);
		...
		norm_to_screen(0, 0) = scale;
		norm_to_screen(0, 2) = (width - scale) / 2;
		norm_to_screen(1, 1) = scale;
		norm_to_screen(1, 2) = (height - scale) / 2;

		// redraw current tab with updated
		// transformation
		redraw();
	}
		...
		switch (key) {
			...
			case 'R':
				if (software_renderer == software_renderer_imp) {
					...
					software_renderer = software_renderer_imp;
				}
	...
	void DrawSVG::draw_diff() {
		...
		for (size_t i = 0; i < width * height; i++) {
			framebuffer[i * 4 + 0] =
				abs(reference[i * 4 + 0] - framebuffer[i * 4 + 0]);
			framebuffer[i * 4 + 1] =
				abs(reference[i * 4 + 1] - framebuffer[i * 4 + 1]);
			framebuffer[i * 4 + 2] =
				abs(reference[i * 4 + 2] - framebuffer[i * 4 + 2]);
	...
	void DrawSVG::draw_zoom() {
		...
		// compute zoom factor---the zoom window should
		// never cover more than 40% of the framebuffer,
		...
		// adjust the cursor coordinates so that the
		// region of interest never goes outside the
		// bounds of the framebuffer
		size_t cX = max(regionSize / 2,
						min(width - regionSize / 2 - 1, (size_t)cursor_x));
		size_t cY = max(regionSize / 2, min(height - regionSize / 2 - 1,
											height - (size_t)cursor_y));
		...
		glReadPixels(cX - regionSize / 2,
					 ...
					 regionSize, regionSize, GL_RGB, GL_UNSIGNED_BYTE,
					 &windowPixels[0]);

		// upsample by the zoom factor, highlighting
		// pixel boundaries
		...
		glDrawPixels(zoomSize, zoomSize, GL_RGB, GL_UNSIGNED_BYTE,
					 &zoomPixels[0]);
		glMatrixMode(GL_PROJECTION);
		glPopMatrix();
		glMatrixMode(GL_MODELVIEW);
	...
	void DrawSVG::redraw() {
		...
		Matrix3x3 m_imp =
			norm_to_screen * viewport_imp[current_tab]->get_svg_2_norm();
		Matrix3x3 m_ref =
			norm_to_screen * viewport_ref[current_tab]->get_svg_2_norm();
